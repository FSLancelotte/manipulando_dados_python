...
salario_previsto = modelo_regressao_logistica.predict(dados_novos_funcionarios)
...
for x in range(len(categoria_salario)):
    categoria = 'Acima da mediana' if categoria_salario == 1 else 'Abaixo da mediana'
    print(f'Categoria de salári previsto do {x+1}º funcionário: R${categoria}')
